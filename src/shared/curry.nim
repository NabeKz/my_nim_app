...
func makeFormedParams(params: NimNode): NimNode =
  result = params
...
func makeCurriedParams(node: NimNode): NimNode =
  result = node
  result.expectKind(nnkFormalParams)
...
func makeCurriedBodyWithCurriedParams(params, body: NimNode): NimNode =
  result = body
...
# template curry*(someProc: untyped){.pragma.}
...
# https://github.com/zer0-star/nim-curry
when isMainModule:
  proc sample(a, b: int): int{.curry.} =
    a + b

  let curried = sample(1)
  echo curried(2)
